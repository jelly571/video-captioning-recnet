...
class CustomVocab(object):
    def __init__(self, caption_fpath, init_word2idx, min_count=1, transform=str.split):
        ...
        #print(self.caption_fpath)只用训练集构造词汇表
        self.min_count = min_count
        self.transform = transform
        #{ '<PAD>': 0, '<SOS>': 1, '<EOS>': 2, '<UNK>': 3 }存入word2idx
        self.word2idx = defaultdict(lambda: init_word2idx['<UNK>'])
        self.word2idx.update(init_word2idx)
        #{ 0:'<PAD>', 1:'<SOS>', 2:'<EOS>', 3:'<UNK>'}存入idx2word
    ...
    def build(self):
        ...
        #构造词汇表
        for word, idx in enumerate(keep_words, len(self.word2idx)):
...
class CustomDataset(Dataset):
    ...
    def load_video_feats(self):
        for model in self.C.feat.models:
            #加载inceptionV4特征，hdf5文件路径
            fpath = self.C.loader.phase_video_feat_fpath_tpl.format(self.C.corpus, model, self.phase)
            #读取h5文件
            fin = h5py.File(fpath, 'r')
            #vid=IpHJffM3G1c_439_446
            for vid in fin.keys():
                feats = fin[vid].value

                # Fix the number of frames for each video 固定每个视频的帧数60
                ...
                # Sample fixed number of frames 样本固定帧数28个
    ...
    def build_video_caption_pairs(self):
        ...
        #print(self.data)
        #print(self.captions)self.captions是一个视频对应好几条caption的，但是self.data中就有多个相同的视频特征，每个只对应一条caption
...
class Corpus(object):
    ...
    def __init__(self, C, vocab_cls=CustomVocab, dataset_cls=CustomDataset):
        ...
        #MSVD,变小写，删除标点符号，对字符串进行切片，不超过30个词
    ...
    def build_data_loaders(self):
        ...
        #在句子开头和结尾加上<SOS><EOS>，对不超过30个词的句子<PAD>
